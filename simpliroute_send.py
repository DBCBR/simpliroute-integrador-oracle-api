...
def get_oracle_view():
    return os.getenv("ORACLE_VIEW") or "VWPACIENTES_ENTREGAS"
...
def fetch_records(limit: int) -> List[Dict[str, Any]]:
    """
    Busca registros da view Oracle definida, limitando a quantidade.
    """
    schema = os.getenv("ORACLE_SCHEMA")
    view = get_oracle_view()
    sql = f"SELECT * FROM {schema}.{view} WHERE ROWNUM <= :limit"
    params = {"limit": limit}
...
def main_loop():
    ...
    while True:
        logger.info("--- INÍCIO DE ENVIO ---")
        try:
            records = fetch_records(SEND_LIMIT)
            if not records:
                logger.info("Nenhum registro encontrado para envio.")
            for idx, record in enumerate(records, 1):
                logger.info(f"Enviando registro {idx}/{len(records)}: reference={record.get('ID_ATENDIMENTO')}")
                # Converter as chaves de record para maiúsculo
                ...
                log_entry = {
                    ...
                    "response": result.get("body", result.get("error")),
                }
                # Salva log estruturado
                log_file = LOG_DIR / f"send_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
